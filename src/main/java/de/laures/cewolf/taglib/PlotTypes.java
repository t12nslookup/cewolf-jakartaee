...
import static org.jfree.chart.renderer.xy.StandardXYItemRenderer.SHAPES;
import static org.jfree.chart.renderer.xy.StandardXYItemRenderer.SHAPES_AND_LINES;
...
public class PlotTypes {
	 ...
	 * @see #typeNames
	 */
	public static final List typeList = asList(typeNames);
	...
	public static AbstractRenderer getRenderer (int idx) {
		switch (idx) {
			...
			case 2: return new StandardXYItemRenderer(SHAPES_AND_LINES);
			case 3: return new StandardXYItemRenderer(SHAPES);
    ...
    public static int getRendererIndex (String plotType) throws AttributeValidationException  {
        var rendererIndex = typeList.indexOf(plotType.toLowerCase());
        if (rendererIndex < 0) {
