 ...
 * 13-Oct-2002 : Corrected error datasetChanged which would generate exceptions when value set to null (BRS).
 ...
 *               added null argument check to setPadding(), setValueFont(), setValuePaint(), setValueFormat()
 *               and setMercuryPaint(), deprecated get/setShowValueLines(), deprecated
 *               getMinimum/MaximumVerticalDataValue(), and fixed serialization bug (DG);
 * 24-Sep-2007 : Implemented new methods in Zoomable interface (DG);
 * 08-Oct-2007 : Added attributes for thermometer dimensions - see patch 1769088 by Julien Henry (DG);
 */
...
import static org.jfree.io.SerialUtilities.readPaint;
import static org.jfree.io.SerialUtilities.readStroke;
import static org.jfree.io.SerialUtilities.writePaint;
import static org.jfree.io.SerialUtilities.writeStroke;
import org.jfree.ui.RectangleEdge;
import org.jfree.ui.RectangleInsets;
import org.jfree.util.ObjectUtilities;
import org.jfree.util.PaintUtilities;
import static org.jfree.util.UnitType.RELATIVE;
...
public class ThermometerPlot extends Plot {
    ...
    /** The bulb radius.  */
    private static final int BULB_RADIUS = 40;

    /** The bulb diameter.  */
    private static final int BULB_DIAMETER = BULB_RADIUS * 2;

    /** The column radius.  */
    private static final int COLUMN_RADIUS = 20;

    /** The column diameter.  */
    private static final int COLUMN_DIAMETER = COLUMN_RADIUS * 2;

    /** The gap radius.  */
    private static final int GAP_RADIUS = 5;

    /** The gap diameter.  */
    private static final int GAP_DIAMETER = GAP_RADIUS * 2;

    /** The axis gap. */
    protected static final int AXIS_GAP = 10;

    /** The unit strings. */
    private static final String[] UNITS = {"", "\u00B0F", "\u00B0C", "\u00B0K"};
    ...
    private transient Paint thermometerPaint = black;
    ...
    private Font valueFont = new Font("SansSerif", BOLD, 16);

    /** Colour that the value is written in */
    private transient Paint valuePaint = white;
    ...
    private transient Paint mercuryPaint = lightGray;
    ...
    private transient Paint[] subrangePaint = {green, orange, red};
    ...
    public ThermometerPlot(ValueDataset dataset) {
        ...
        this.padding = new RectangleInsets(RELATIVE, 0.05, 0.05, 0.05, 0.05);
        this.dataset = dataset;
        if (dataset != null) {
            dataset.addChangeListener(this);
        }
        var axis = new NumberAxis(null);
        axis.setStandardTickUnits(createIntegerTickUnits());
    ...
    public void setDataset(ValueDataset dataset) {
        ...
        var event = new DatasetChangeEvent(this, dataset);
    ...
    *
    ...
    public void draw(Graphics2D g2, Rectangle2D area, Point2D anchor,
        ...
        // adjust for padding...
        var interior = (Rectangle2D) area.clone();
        this.padding.trim(interior);
        var midX = (int) (interior.getX() + (interior.getWidth() / 2));
        var midY = (int) (interior.getY() + (interior.getHeight() / 2));
        var stemTop = (int) (interior.getMinY() + getBulbRadius());
        var stemBottom = (int) (interior.getMaxY() - getBulbDiameter());
        ...
        var outerThermometer = new Area(outerBulb);
        var tempArea = new Area(outerStem);
        ...
        var innerThermometer = new Area(innerBulb);
        tempArea = new Area(innerStem);
        innerThermometer.add(tempArea);

        if ((this.dataset != null) && (this.dataset.getValue() != null)) {
            var current = this.dataset.getValue().doubleValue();
            var ds = this.rangeAxis.valueToJava2D(current, dataArea, RectangleEdge.LEFT);
            var i = getColumnDiameter() - getGap() * 2; // already calculated
            var j = getColumnRadius() - getGap(); // already calculated
            var l = (i / 2);
            var k = (int) round(ds);
            ...
            var mercury = new Area(innerBulb);
            ...
            if (this.subrangeIndicatorsVisible) {
                ...
                var range = this.rangeAxis.getRange();
                // draw start of normal range
                var value = this.subrangeInfo[NORMAL][RANGE_LOW];
                if (range.contains(value)) {
                    double x = midX + getColumnRadius() + 2;
                    var y = this.rangeAxis.valueToJava2D(value, dataArea, RectangleEdge.LEFT);
            ...
            switch (this.valueLocation) {
                case RIGHT:
                    ...
                    break;
                case LEFT:
                    var valueString = this.valueFormat.format(current);
                    var stringWidth = metrics.stringWidth(valueString);
        ...
        metrics = g2.getFontMetrics();
        var tickX1 = midX - getColumnRadius() - getGap() * 2 - metrics.stringWidth(UNITS[this.units]);
    ...
    public void datasetChanged(DatasetChangeEvent event) {
        if (this.dataset != null) {
            var vn = this.dataset.getValue();
            if (vn != null) {
                var value = vn.doubleValue();
    ...
    private Paint getCurrentPaint() {
        var result = this.mercuryPaint;
        if (this.useSubrangePaint) {
            var value = this.dataset.getValue().doubleValue();
            if (inSubrange(NORMAL, value)) {
    ...
    public boolean equals(Object obj) {
        ...
        for (var i = 0; i < this.subrangePaint.length; i++) {
    ...
    public Object clone() throws CloneNotSupportedException {
        ...
        var clone = (ThermometerPlot) super.clone();
    ...
    private void writeObject(ObjectOutputStream stream) throws IOException {
        ...
        writeStroke(this.thermometerStroke, stream);
        writePaint(this.thermometerPaint, stream);
        writePaint(this.valuePaint, stream);
        writePaint(this.mercuryPaint, stream);
        writeStroke(this.subrangeIndicatorStroke, stream);
        writeStroke(this.rangeIndicatorStroke, stream);
        for (var i = 0; i < 3; i++) {
            writePaint(this.subrangePaint[i], stream);
        }
    }
    ...
    private void readObject(ObjectInputStream stream) throws IOException, ClassNotFoundException {
        ...
        this.thermometerStroke = readStroke(stream);
        this.thermometerPaint = readPaint(stream);
        this.valuePaint = readPaint(stream);
        this.mercuryPaint = readPaint(stream);
        this.subrangeIndicatorStroke = readStroke(stream);
        this.rangeIndicatorStroke = readStroke(stream);
        this.subrangePaint = new Paint[3];
        for (var i = 0; i < 3; i++) {
            this.subrangePaint[i] = readPaint(stream);
        }
    ...
    public void zoomRangeAxes(double factor, PlotRenderingInfo state, Point2D source, boolean useAnchor) {
        var anchorY = this.getRangeAxis().java2DToValue(source.getY(),
